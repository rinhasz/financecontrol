...
def _gravar(conn, data):
    if data.get('id'):
        conn.execute("""
            ...
            padrao_variabilidade=?, valor_padrao=?, regras_match=?, recorrencia=?, varios_por_mes=?, tipo_projecao=?, valor_projecao=?, ativo=? WHERE id=?
        """, (data['nome'], data.get('categoria_id'), data.get('dia_vencimento'),
              ...
              data.get('tipo_projecao', 'media_movel_6'), float(data.get('valor_projecao') or 0), data.get('ativo', 1), data['id']))
        despesa_id = data['id']
    else:
        cur = conn.execute("""
            INSERT INTO despesa (nome, categoria_id, dia_vencimento, tipo_valor, padrao_variabilidade, valor_padrao, recorrencia, varios_por_mes, tipo_projecao, valor_projecao, regras_match)
            VALUES (?,?,?,?,?,?,?,?,?,?,?)
        """, (data['nome'], data.get('categoria_id'), data.get('dia_vencimento'),
              data.get('tipo_valor', 'variavel'), data.get('padrao_variabilidade', 'variavel_nao_sazonal'),
              ...
              float(data.get('valor_projecao') or 0),
